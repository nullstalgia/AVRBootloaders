...
void Application_Jump_Check(void) {
  ...
  */
  ...
  if (pgm_read_word(0) != 0xFFFF) {
    // Looks like it.
    // Were we reset by a button? (most likely case for EXTRF)
    if ((mcusr_state & (1 << EXTRF))) {
      // Okay, then is the magic key set?
      if ((MagicBootKeyPtrVal != MAGIC_BOOT_KEY)) {
        // No. But let's set it ourselves, and see if the user hits the button again
        *MagicBoot_Key_Ptr = MAGIC_BOOT_KEY;

        // wait for a possible double tap (this method takes less flash than an
        // ISR)
        _delay_ms(EXT_RESET_TIMEOUT_PERIOD);

        // user was too slow, or it was something else.
        *MagicBoot_Key_Ptr = 0;

        // Single tap reset, start sketch
        if (DOUBLE_TAP_RESET_TO_BOOTLOADER) {
          StartSketch();
        }
      } else if (!DOUBLE_TAP_RESET_TO_BOOTLOADER) {
        // Little odd case if the key is set, and if they don't want double-tapping to work.
        StartSketch();
      }
    }
    ...
    // On a power-on reset, we ALWAYS want to go to the sketch. If there is one.
      ...
      if (!POWER_ON_TO_BOOTLOADER) {
        // MagicBootKey = 0;
        StartSketch();
        //LEDs_SetAllLEDs(LEDS_LED2|LEDS_LED1);
        //_delay_ms(1000);
      }
    }
...
int main(void) {
  *MagicBoot_Key_Ptr = 0;
   ...
   */
  while (RunBootloader || WaitForExit) USB_USBTask();

  /* Wait a short time to end all USB transactions and then disconnect */
  _delay_us(1000);

  /* Reset configured hardware back to their original states for the user
   * application */
  ResetMCU();
...
/** ISR to periodically toggle the LEDs on the board to indicate that the
        ...
        if (!(SentCommand.DataSize)) {
          ...
          if (IS_ONEBYTE_COMMAND(SentCommand.Data, 0x00))  // Write flash
          {
            ...
            while (WordsRemaining--) {
              /* Check if endpoint is empty - if so clear it and wait until
    ...
    case DFU_REQ_UPLOAD:
      ...
      if (DFU_State != dfuUPLOAD_IDLE) {
        ...
        if (IS_ONEBYTE_COMMAND(SentCommand.Data, 0x00))  // Read FLASH
        {
          ...
          while (WordsRemaining--) {
            /* Check if endpoint is full - if so clear it and wait until ready
             * for next packet */
  ...
  if (IS_ONEBYTE_COMMAND(SentCommand.Data, 0x03))  // Start application
  {
    ...
    if (SentCommand.DataSize) {
      ...
      } else  // Start via jump
      {
        ...
        if (pgm_read_word_near(0) == 0xFFFF) RunBootloader = false;
      }
    }
